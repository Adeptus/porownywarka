...
class SearchesController extends Zend_Controller_Action {
	...
	public function indexAction() {
  		if (($this->_getParam('monitor_id') > 0) && ($this->_getParam('monitor2_id') > 0)) {
			$monitor   = $this->show_one($this->_getParam('monitor_id'));
			$monitor2  = $this->show_second($this->_getParam('monitor2_id'));
	...
	public function searchAction() {
		...
		if (($this->requestIsAPostAndHasValidParams($form)) && ($this->getRequest()->getParam('id'))) {
			$this->show_one($this->_getParam('id'));
	...
	private function show_one($monitor_id) {
	...
	private function show_second($monitor_id) {
